...
// 重点修改：切换到和知乎/B站相同的 PearAPI 源，并修复报错显示
async function fetchKuaishou() {
  ...
  try {
    // 改用 PearAPI，稳定性同知乎/B站
    const url = "https://api.pearktrue.cn/api/dailyhot/?title=" + encodeURIComponent("快手");
    const resp = await httpGet(url);
    const json = parseJSON(resp.body, name);
    ...
    if (!Array.isArray(data)) {
      // 抛出详细错误，防止 [object Object]
      throw new Error(`数据格式错误: ${JSON.stringify(json).slice(0, 100)}`);
    }
    ...
    // 强制把错误转为字符串，防止 [object Object]
    const errMsg = e.message || JSON.stringify(e);
    log(`${name} 获取失败：${errMsg}`);
    return { ok: false, title: name, err: errMsg };
  }
}
