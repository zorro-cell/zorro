...
// 更新日期: 2025年12月13日（B站接口顺序微调：优先 pearktrue -> 官方 popular -> 官方 ranking -> 第三方兜底）
// 修复: B站通知不再强制跳转搜索；优先官方“综合热门”接口，保留视频直达链接
...
const PLATFORMS = {
  ...
  bilibili: {
    name: 'B站热门',
    // home 保持可打开热门页
    home: `bilibili://browser?url=${encodeURIComponent("https://www.bilibili.com/v/popular/all")}`,
    urls: [
      // ✅ QX 脚本里用的 pearktrue 聚合（优先）
      'https://api.pearktrue.cn/api/dailyhot/?title=哔哩哔哩',
      // ✅ 官方综合热门（视频列表）
...
function normalizeList(platformName, rawData) {
  ...
  items = items.map((item) => {
    ...
    if (platformName === '微博热搜') {
      ...
      // ✅ 修复：B站不再无条件改成搜索；有视频直达链接就保留并转为 browser scheme
      if (!url) {
